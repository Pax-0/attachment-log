...
module.exports.generator = async (msg, args) => {
	...
	if(!attacthmentLog) return msg.channel.createMessage('I Couldnt find the channel for attachment logs.');
...
async function sendControlPanelEmbeds(channel, member){
	const welcomeMSGEmbed = makeEmbed('The welcome module is enabled', `Last toggled by: ${member.mention}`, bot.user.username, '3066993', channel.guild.iconURL);
	let welcomeLogMSG = await channel.createMessage({embed: welcomeMSGEmbed});
	await welcomeLogMSG.addReaction('🔁');

	const attacthmentLogMSGEmbed = makeEmbed('The attachmentlog module is enabled', `Last toggled by: ${member.mention}`, bot.user.username, '3066993', channel.guild.iconURL);
	...
	await bot.db.settings.update({}, { $addToSet: { 'controlPanel.messages': {msgId: attatchmentLogMSG.id, module: 'attachmentlog'} } }); 
...
module.exports.options = {
	...
	usage:'#attachment-log #control-panel #welcome-channel',
	argsRequired: true,
	guildOnly: true,
	requirements: {
